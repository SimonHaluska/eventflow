...
const defaultSocialLinks = [
  { label: "Instagram", href: "https://www.instagram.com/momentumevents.sk" },
  { label: "Facebook", href: "https://www.facebook.com/momentumevents.sk" },
  { label: "LinkedIn", href: "https://www.linkedin.com/company/momentum-events-sk" },
  { label: "TikTok", href: "https://www.tiktok.com/@momentumevents.sk" },
];
...
export default function Footer({ dict, lang, siteSettings }: Props) {
  const year = new Date().getFullYear();
  const socialLinks =
    siteSettings?.socialLinks.length ? siteSettings.socialLinks : defaultSocialLinks;
  const mapUrl = siteSettings?.mapEmbedUrl || defaultMapUrl;
  const email = siteSettings?.email || "info@momentumevents.sk";

  return (
    <footer className="bg-charcoal px-6 py-16 text-background">
      <div className="mx-auto max-w-5xl space-y-10">
        <div className="grid grid-cols-1 gap-x-10 gap-y-10 sm:grid-cols-4 sm:items-start">
          <div>
            <Link href={`/${lang}`} className="inline-block sm:-translate-y-[15px]">
              <Image
                src="/logo.png"
                alt="Momentum Events"
                width={543}
            ...
            {siteSettings?.openingHours && (
              <div className="mt-6">
                ...
                <p className="mt-2 whitespace-pre-line text-sm text-background/60">
                  {siteSettings.openingHours}
                </p>
              </div>
            )}
          ...
          <div className="flex flex-col gap-4">
            ...
            {siteSettings?.address && (
              <p className="text-sm text-background/60">{siteSettings.address}</p>
            )}
